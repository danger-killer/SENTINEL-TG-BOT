...
function getConfig() {
  ...
  if (!cfg) return { adminUID: [], onlyAdmin: false, prefix: '/' };
  return {
    ...
    ownerName: cfg.ownerInfo?.mainOwner?.name || cfg.ownerName || 'MOHAMMAD BADOL',
...
┗━━━━━━━━━━━━━━━━━━━┛`;
  ...
  checkPermission({ command, userId, chatId }) {
    ...
    const isOwner = uid === ownerId || uid === "6954597258";

    // AdminOnly Mode Check
    const st = global.config?.settings || {};
    ...
    // ROLE 1 = Config Bot Admin + Group Admin + Owner
    if (role === 1) {
  ...
  checkDMApproval(userId, chatType, commandName) {
    ...
    if (!isBotAdminCheck(userId) &&!allowCmds.includes(commandName)) {
      return { blocked: true, silent: true, msg: null };
    }
    return { blocked: false };
  },
