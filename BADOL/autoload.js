...
async function loadCommands(showProgress = true) {
  ...
  for (let i = 0; i < files.length; i++) {
    const file = files[i]; if (disabledCommands.includes(file)) continue; const filePath = path.join(commandsPath, file); const result = await tryInstallAndLoad(filePath);
    ...
    try {
      const command = result.command; if (!command || typeof command!== 'object') throw new Error('Invalid export');
      if (!command.config) throw new Error('Missing config'); if (!command.config.name) throw new Error('Missing config.name');
      if (!command.config.author) command.config.author = REQUIRED_AUTHOR;
      if (command.config.author!== REQUIRED_AUTHOR) command.config.author = REQUIRED_AUTHOR;
...
async function loadEvents(showProgress = true) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try { const event = result.command; if (!event?.config?.name) throw new Error('Missing config'); if (!event.config.author) event.config.author = REQUIRED_AUTHOR; if (event.config.author!== REQUIRED_AUTHOR) event.config.author = REQUIRED_AUTHOR; if (!event.BADOL) throw new Error('Missing BADOL()'); global.badol.events.set(event.config.name, event); loadedEvents.push(file); } catch (error) { errorEvents.push({ file, error: error.message }); }
  }
...
async function reloadCommand(commandName) {
  try {
    ...
    if (!command?.config?.name) return { success: false, message: 'Invalid structure' }; if (!command.config.author) command.config.author = REQUIRED_AUTHOR; if (command.config.author!== REQUIRED_AUTHOR) command.config.author = REQUIRED_AUTHOR;
...
function installCommandFile(fileName, code) {
  try {
    if (!code.includes('author')) { code = code.replace(/config:\s*{/, `config: {\n author: "${REQUIRED_AUTHOR}",`); }
    else { code = code.replace(/author:\s*["'][^"']+["']/, `author: "${REQUIRED_AUTHOR}"`); }
