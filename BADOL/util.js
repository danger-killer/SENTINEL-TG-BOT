...
// CACHE PATH - FIXED FOR NEW SYSTEM
...
function checkPermission(userId, role, chatId = null) {
  ...
  if (role === 2) return global.config.adminUID.includes(String(userId));
  if (role === 1) {
    if (global.config.adminUID.includes(String(userId))) return true;
    if (chatId && global.badol?.threadAdmins?.has(String(chatId))) {
      const cached = global.badol.threadAdmins.get(String(chatId));
      return cached.admins.includes(String(userId));
    }
    return false;
  }
  return false;
...
module.exports = {
  MessageUtils, getTime, formatUptime, checkPermission, shortenURL, uploadImgbb,
  getStreamFromURL, downloadFile, randomString, fetchUserData, STBotApis,
