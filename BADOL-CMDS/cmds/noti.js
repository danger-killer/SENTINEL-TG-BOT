  ...
  BADOL: async function ({ event, api, args, message, userId }) {
    try {
      ...
      if (sub === 'dm') {
        ...
        const dmUsers = users.filter(u => u.dmApproved);
        if (!dmUsers.length) return message.reply('❌ কোনো DM ইউজার নাই।');
        let txt = `📋 DM Users (${dmUsers.length}):\n\n`;
        dmUsers.slice(0, 50).forEach((u, i) => {
      ...
      // Error হাইড, handler এ ❌ দেখাবে না
    }
  },
  ...
  sendBroadcast: async function (api, message, event, notiMessage, mode = 'all', targets = []) {
    ...
    if (mode === 'all') {
      try {
        ...
        const users = await global.db.getAllUsers();
        recipients = [
         ...threads.filter(t => t.type === 'group' || t.type === 'supergroup').map(t => t.id),
         ...users.filter(u => u.dmApproved).map(u => u.id)
        ];
      } catch {}
    } else {
      recipients = targets;
