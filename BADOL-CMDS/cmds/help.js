...
module.exports = {
  ...
  BADOL: async function ({ event, api, args, message, chatId }) {
    ...
    const botName = global.config.botName || "BADOL-TG-BOT";
    const developer = global.config.ownerName || "MOHAMMAD-BADOL";
  ...
  onCallback: async function ({ event, api, ctx }) {
    try {
      ...
      if (!stored) stored = { page: 1, totalPages: 1, prefix: global.config.prefix, botName: global.config.botName, developer: global.config.ownerName, HELP_IMG: "https://drive.google.com/uc?export=download&id=1iLb2BSW5jzF0shxkLvylEXkhJmX-BIaH" };
