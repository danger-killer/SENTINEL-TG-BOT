...
module.exports = {
  ...
  BADOL: async function ({ event, api, args, message, chatId, ctx }) {
    ...
    const MODEL_VERSION = "badol-dslr-beauty-v9-natural";
    ...
    // Image URL বের করা - info.js এর মতো
    let imageUrl = null;
    let fileId = null;

    if (event.reply_to_message?.photo) {
      const photos = event.reply_to_message.photo;
      fileId = photos[photos.length - 1].file_id; // সবচেয়ে বড় সাইজ
    } else if (event.photo) {
...
╰─────────────────╯`
    ...
    try {
      // file_id থেকে direct link - info.js এর মতো
