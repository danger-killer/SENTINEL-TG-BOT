module.exports = {
  ...
  BADOL: async function ({ event, api, args, message, chatId, userId, ctx }) {
    ...
    global.badol.onCallback.set(`confirm_gcname_${chatId}`, {
    ...
    global.badol.onCallback.set(`cancel_gcname_${chatId}`, {
      commandName: 'setgcname',
      userId: userId,
      chatId: chatId
    });
  ...
  onCallback: async function ({ event, api, message, ctx }) {
    ...
    if (data.startsWith('confirm_gcname_')) {
      try {
        ...
        global.badol.onCallback.delete(data);
        global.badol.onCallback.delete(`cancel_gcname_${chatId}`);
    ...
    if (data.startsWith('cancel_gcname_')) {
      global.badol.onCallback.delete(`confirm_gcname_${chatId}`);
      global.badol.onCallback.delete(data);
